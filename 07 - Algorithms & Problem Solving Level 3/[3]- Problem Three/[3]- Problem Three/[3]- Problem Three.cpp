...
int summationEachRowInMatrix(int* matrix[], int& row, int& column) {

    int summationEachRow{ 0 };

    for (int counterI{ 0 }; counterI < column; counterI++)
            summationEachRow += matrix[row][counterI];

        return summationEachRow; 
}


void print1DArraySumationEachRow( int* matrix[], int row , int column ) {

    for (int counter{ 0 }; counter < row; counter++)
    {
        cout << "Sum Row " << counter + 1;


        // space if the rows total more than 5 spaces     
        int result = summationEachRowInMatrix(matrix, counter, column); 
        printf(" = [%-6d]\n", result);
    }
}
...
int main()
{
    ...
    print1DArraySumationEachRow(matrix, row, column);
    cout << endl << endl;

...
void printArrayDynamic(int* matrix[], int row, int column) {
    ...
    for (int counter{ ::kZERO }; counter < row; counter++) {
        ...
        for (int counterJ{ ::kZERO }; counterJ < column; counterJ++)
            printf("%03d ", matrix[counter][counterJ]);
...
void multiplyingTwoMatrix(int* matrixOne[], int* matrixTwo[], int* matrixResultMulitply[], int row, int column) {
    ...
    int numberResultMultiplying{ 0 };
    for (int counterOut{ 0 }; counterOut < row; counterOut++) {


        int counterI{ 0 };
        for (; counterI < row; counterI++) {

            for (int counterJ{ 0 }; counterJ < column; counterJ++) {
                numberResultMultiplying += matrixOne[counterI][counterJ] * matrixTwo[counterJ][counterOut];
            }

            matrixResultMulitply[counterI][counterOut] = numberResultMultiplying;
            numberResultMultiplying = { 0 };
        }
        }
}

bool ceheakNumberEven(int& number) {
    return (number % 2 == 0); 
}
bool ceheakNumberOdd(int& number) {
    return (number % 2 != 0);
}
int main()
{
    ...
    if ((ceheakNumberEven(row) && ceheakNumberEven(column)) || ( ceheakNumberOdd (row) && ceheakNumberOdd(column))) {
        ...
        cout << "\n\nPlease sir , Enter the Row and Column is Square numbers  \n";
}

...
void storeSummationColumnInArray(int* matrix[], int& row, int& column, int* arrayColumn) {
    for (int counterI{ 0 }; counterI < row; counterI++)
        *(arrayColumn + counterI) = summationEachColumnInMatrix(matrix, column, counterI);
    
}

void print1DArray(int* arrayOfColumn, int sizeArray1D) {
...
int main()
{
    ...
    storeSummationColumnInArray(matrix, row, column, array1D);
    print1DArray( array1D , row );
    cout << endl << endl;
